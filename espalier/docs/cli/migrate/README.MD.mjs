...
\`## Usage\` block listing every flag \`espalier migrate\` accepts. A reader
looking up a command should not have to read the parser.

The page and the implementation are two halves of one claim. \`src/migrate.ts\`
...
export async function lint({ read, files, emit }) {
  ...
  const heading = "# `espalier migrate`";
  ...
  if ((await files("src/migrate.ts")).length === 0) {
    emit({
      code: "undocumented_command",
      message: "documents \`migrate\`, which src/migrate.ts does not implement",
      metadata: { command: "migrate" },
    });
  }
}

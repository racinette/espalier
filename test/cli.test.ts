...
test("--version prints the installed package version without loading the repository", () => {
  scratch((root) => {
    writeFileSync(path.join(root, "espalier.config.yaml"), "not: valid: yaml\n");
    const printed = run(root, ["--version"]);
